...
class TestListPermissionsUseCase:
    ...
    async def test_should_return_all_permissions(
        self,
        ...
        for p in perms:
            await permission_in_memory_repo.save(p)

        input_dto = ListRequestInputDTO(actor=admin_actor)
        result = await list_permissions_use_case.execute(input_dto)
    ...
    async def test_should_return_empty_list_if_no_permissions(
        self,
        list_permissions_use_case,
        admin_actor: User,
    ):
