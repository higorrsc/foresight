...
def permission_repository(db_session_for_test):
    """
...
class TestPermissionRepository:
    ...
    def test_save_and_get_by_codename(
        ...
        found_permission = permission_repository.get_by_codename(
            codename="test.permission"
        )
    ...
    def test_list_all(
        ...
        initial_permissions = permission_repository.list_all()
        ...
        permission_repository.save(perm1)
        permission_repository.save(perm2)

        # 2. List all and check if the count has increased by 2
        all_permissions = permission_repository.list_all()
    ...
    def test_get_by_codename_not_found(self, permission_repository):
        """Test that get_by_codename returns None for a non-existent permission."""
        found_permission = permission_repository.get_by_codename("non.existent.perm")
