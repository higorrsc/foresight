...
class TestPermissionRepository:
    ...
    async def test_save_and_get_by_codename(
        self,
        ...
        await permission_sqlalchemy_repo.save(permission)

        found_permission = await permission_sqlalchemy_repo.get_by_codename(
            codename="test.permission"
        )
    ...
    async def test_list_all(
        ...
        initial_permissions = await permission_sqlalchemy_repo.list_all()
        ...
        await permission_sqlalchemy_repo.save(perm1)
        await permission_sqlalchemy_repo.save(perm2)

        # 2. List all and check if the count has increased by 2
        all_permissions = await permission_sqlalchemy_repo.list_all()
    ...
    async def test_get_by_codename_not_found(self, permission_sqlalchemy_repo):
