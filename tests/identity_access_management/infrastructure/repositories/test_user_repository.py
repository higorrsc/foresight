...
class TestUserRepository:
    ...
    async def test_save_and_get_by_id(
        ...
        saved_user = await user_sqlalchemy_repo.save(user)

        assert saved_user is not None
        assert saved_user.id == user.id

        found_user = await user_sqlalchemy_repo.get_by_id(
            entity_id=user.id,
            tenant_id=default_tenant_id,
        )
    ...
    async def test_get_by_username_found(
        ...
        await user_sqlalchemy_repo.save(user)

        found_user = await user_sqlalchemy_repo.get_by_username(
            username="findme",
            tenant_id=default_tenant_id,
        )
