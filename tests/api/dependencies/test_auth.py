...
def test_get_auth_provider_local(mock_session):
...
@pytest.mark.anyio
async def test_get_current_user_success(mock_provider):
    token = "valid_token"
    user = Mock(spec=User)
...
@pytest.mark.anyio
async def test_get_current_user_failure(mock_provider):
    token = "invalid_token"
