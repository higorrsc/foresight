...
class TestGetOrganizationalUnitByIdUseCase:
    ...
    async def test_get_organizational_unit_by_id_success(
        self,
        ...
        result = await get_organizational_unit_by_id_use_case.execute(input_dto)

        assert result == entity
        assert result.id == entity_id

    async def test_get_organizational_unit_by_id_insufficient_permission(
        self,
        get_organizational_unit_by_id_use_case,
        admin_actor,
    ):
        ...
        with pytest.raises(InsufficientPermissionError):
            await get_organizational_unit_by_id_use_case.execute(input_dto)
    ...
    async def test_get_organizational_unit_by_id_not_found(
        ...
        """
        guest_actor.permissions.add(AppPermission.ORGANIZATIONAL_UNIT_READ)
