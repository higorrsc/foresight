...
class TestGetAreaByIdUseCase:
    ...
    def test_get_area_by_id_with_valid_id(
        self, admin_actor, area_in_memory_repo, get_area_by_id_use_case
    ):
        """
        ...
        retrieved_area = get_area_by_id_use_case.execute(
    ...
    def test_get_area_by_id_with_non_existent_id(
        self, admin_actor, get_area_by_id_use_case
    ):
        """
        ...
        with pytest.raises(AreaNotFoundError, match="Area with given ID not found."):
            get_area_by_id_use_case.execute(
                GetByIdRequestInputDTO(
                    id=non_existent_id,
                    actor=admin_actor,
                )
